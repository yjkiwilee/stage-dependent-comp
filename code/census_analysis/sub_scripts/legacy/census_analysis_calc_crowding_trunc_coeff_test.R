...
# Decay function to use; a truncated Gaussian which becomes zero as effect = 0.01
DECAY_FUN <- function(x, a) {
  ifelse(x <= a, exp(x^2 / a^2 * -4.60517), 0)
}
...
for(i in 1:nrow(focal_plts)) {
  ...
  if(NA %in% plot_sub_data$Sp1.crowding) {
    print("NA DETETCTED")
    print(plot_sub_data)
    print(calc_dist_mat(plot_sub_data))
  }
