...
# ===== State variables carried over from run script =====
# MODEL_TYPE = c("lmm", "glmm")
# ALPHA = numeric()
...
stat_fitted <- if(MODEL_TYPE == "lmm") {
  ...
  list(
    "SP1.S" = fit_vr_glmm(stat_model_full_list, "S", sp_stage_data_norm$sp1_s,
                          maxit = 7e4),
    "SP1.L" = fit_vr_glmm(stat_model_full_list, "L", sp_stage_data_norm$sp1_l,
                          maxit = 7e4),
    "SP2.S" = fit_vr_glmm(stat_model_full_list, "S", sp_stage_data_norm$sp2_s,
                          maxit = 7e4),
    "SP2.L" = fit_vr_glmm(stat_model_full_list, "L", sp_stage_data_norm$sp2_l,
                          maxit = 7e4)
  )
}
...
# DF for storing model predictions
...
for(model_class in names(stat_fitted)) {
  for(model_vr in names(stat_fitted[[model_class]])) {
    for(model_name in names(stat_fitted[[model_class]][[model_vr]])) {
      ...
      # If the model is NA, indicating an error, skip over
      if(is.na(model_temp)) { next }
      ...
      # Calculate MSE
      mse_temp <- mean((stat_predictions$Vital.rate.value - stat_predictions$Predict) ^ 2, na.rm = TRUE)
      
      # Insert MSE into mse df
      mse_df <- bind_rows(mse_df, tibble(
        # Key for mapping onto results
        Key = paste(
          strsplit(model_class, "\\.")[[1]][1],
          model_stage,
          model_vr,
          model_name,
          sep = "_"
...
ggsave(here("StageDependentComp", "figures", "census_analysis", "crowding",
            "crowding_factor_test", paste0("data_predict_", MODEL_TYPE,
                                           "_", ALPHA, ".png")),
       width = 20, height = 12)
...
stat_summ_temp_df <- stat_summ_rescaled_df %>%
  ...
  dplyr::group_by(Taxon, Stage, Vital.rate, Model.name) %>%
  dplyr::summarise(
  ...
  dplyr::ungroup() %>%
  dplyr::select(-one_of(c("Taxon", "Stage", "Vital.rate", "Model.name")))

# Calculate intercepts
stat_summ_rescaled_df <- stat_summ_rescaled_df %>%
  dplyr::mutate(
    Key = paste(
      Taxon,
      Stage,
  ...
  dplyr::left_join( # Join intercept stats by Key
    stat_summ_temp_df,
    by = join_by(Key)
  ) %>%
  dplyr::select(-Key) %>% # Remove Key
  dplyr::mutate(
  ...
  dplyr::ungroup() %>%
  dplyr::select(-one_of(c("Coefficient.trans", "Lower.95CI.trans", "Upper.95CI.trans"))) # Remove trans vars

# Merge MSE
stat_summ_rescaled_df <- stat_summ_rescaled_df %>%
  dplyr::mutate( # Create Key to map MSE
    Key = paste(
      Taxon,
      Stage,
      Vital.rate,
  ...
  left_join(mse_df, by = join_by(Key)) %>% # Map MSE
  dplyr::select(-Key) # Drop Key

# Store summary as a csv
write_csv(stat_summ_rescaled_df,
          here("StageDependentComp", "result_data", "wp2",
               "crowding_factor_test",
               paste0("stat_summary_full_", MODEL_TYPE, "_", ALPHA, ".csv")))
