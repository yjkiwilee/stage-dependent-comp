...
sp1_vrs <- list(
  s = c(0.23, 0.95), # Small & large survival
  g = c(0.4), # Maturation rate
...
# Density-independent vital rates of species 1 (Fast lh species)
sp2_vrs <- list(
  s = c(0.22, 0.95), # Small & large survival
  g = c(0.4), # Maturation rate
  r = c(0.1), # Retrogression rate
  f = c(1.5), # Fecundity
  p = c(1) # Recruitment factor
...
# ggsave(here("StageDependentComp","figures","comm_sims","stoch_slowlh_varstdep_mean_mult.png"), width = 8, height = 5)
