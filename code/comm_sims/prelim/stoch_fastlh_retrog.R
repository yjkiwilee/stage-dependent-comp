...
sp1_vrs <- list(
  s = c(0.7, 0.1), # Seed, juvenile, adult survival
  g = c(0.9), # Maturation rate
  r = c(0.9), # Retrogression rate
  f = c(2), # Fecundity
...
sp2_vrs <- list(
  s = c(0.8, 0.1), # Seed, juvenile, adult survival
  g = c(0.9), # Maturation rate
  r = c(0.9), # Retrogression rate
  f = c(2), # Fecundity
  p = c(1) # Recruitment factor
...
st_dep_m <- alter_k_mat(st_dep_m, vr_type = "s", mat_row = 3, mat_col = 1, alt_val = 0.00025, iter = 70, scal_fact_l = -100, scal_fact_r = 100)
st_dep_m <- alter_k_mat(st_dep_m, vr_type = "s", mat_row = 3, mat_col = 3, alt_val = 0.0007, iter = 70, scal_fact_l = -100, scal_fact_r = 100)
