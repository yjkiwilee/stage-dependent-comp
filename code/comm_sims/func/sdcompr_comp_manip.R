...
#' 
#' @export
calc_avg_int_str_rb <- function(sdcomp_model, vr_type, mat_row, species, timestep = SIMEQ_TIMESTEP_ROBUST, sp1_0 = DEFAULT_POP_STRUCT, sp2_0 = DEFAULT_POP_STRUCT, avgtf = SIMEQ_AVGTF, dstb_func = NULL, vrstoch_func = NULL) {
  n_stages <- sdcomp_model$n_stages
  
  # Find 'equilibrium' of species with current parameters
  end_state <- sdcomp_project_end_avg(sdcomp_model, sp1_0, sp2_0, timestep, avgtf, dstb_func, vrstoch_func)
...
#' 
#' @export
calc_avg_int_str_mult <- function(sdcomp_model, vr_type, mat_rows, species, timestep = SIMEQ_TIMESTEP_ROBUST, sp1_0 = DEFAULT_POP_STRUCT, sp2_0 = DEFAULT_POP_STRUCT, avgtf = SIMEQ_AVGTF, dstb_func = NULL, vrstoch_func = NULL) {
...
#' 
#' @export
alter_k_mat_rb <- function(sdcomp_model, vr_type, mat_row, mat_col, alt_val, timestep = SIMEQ_TIMESTEP_ROBUST, avgtf = SIMEQ_AVGTF, iter = 40, scal_fact_l = 0, scal_fact_r = 5, sp1_0 = DEFAULT_POP_STRUCT, sp2_0 = DEFAULT_POP_STRUCT, dstb_func = NULL, vrstoch_func = NULL) {
  n_stages <- sdcomp_model$n_stages
  species <- (mat_col - 1) %/% n_stages + 1 # 1 if species 1, 2 if species 2
  
  # Determine the target per-capita interaction strength
  targ_int_str <- calc_avg_int_str_rb(sdcomp_model, vr_type, mat_row, species, timestep = timestep, avgtf = avgtf, sp1_0 = sp1_0, sp2_0 = sp2_0, dstb_func = dstb_func, vrscoth_func = vrstoch_func)
  ...
  for(i in 1:iter) {
    ...
    l_int_str <- calc_avg_int_str_rb(l_model, vr_type, mat_row, species, timestep, avgtf, sp1_0, sp2_0, dstb_func, vrstoch_func)
    r_int_str <- calc_avg_int_str_rb(r_model, vr_type, mat_row, species, timestep, avgtf, sp1_0, sp2_0, dstb_func, vrstoch_func)
    m_int_str <- calc_avg_int_str_rb(m_model, vr_type, mat_row, species, timestep, avgtf, sp1_0, sp2_0, dstb_func, vrstoch_func)
...
#' 
#' @export
alter_k_mat_mult <- function(sdcomp_model, vr_type, mat_rows, mat_cols, alt_vals, timestep = SIMEQ_TIMESTEP_ROBUST, avgtf = SIMEQ_AVGTF, iter = 40, scal_fact_l = 0, scal_fact_r = 5, sp1_0 = DEFAULT_POP_STRUCT, sp2_0 = DEFAULT_POP_STRUCT, dstb_func = NULL, vrstoch_func = NULL) {
  ...
  targ_int_strs <- calc_avg_int_str_mult(sdcomp_model, vr_type, mat_rows, species, timestep = timestep, avgtf = avgtf, sp1_0 = sp1_0, sp2_0 = sp2_0, dstb_func = dstb_func, vrstoch_func = vrstoch_func)
  ...
  for(i in 1:iter) {
    ...
    if((FALSE %in% (scal_fact_ls_prev == scal_fact_ls)) | (TRUE %in% is.na(scal_fact_ls_prev))) { # Calculate l_int_strs only if scal_fact_ls has been updated or hasn't been calculated before
      l_int_strs <- calc_avg_int_str_mult(l_model, vr_type, mat_rows, species, timestep = timestep, avgtf = avgtf, sp1_0 = sp1_0, sp2_0 = sp2_0, dstb_func = dstb_func, vrstoch_func = vrstoch_func)
    }
    if((FALSE %in% (scal_fact_rs_prev == scal_fact_rs)) | (TRUE %in% is.na(scal_fact_ls_prev))) { # Calculate r_int_strs only if scal_fact_rs has beeen updated or hasn't been calculated before
      r_int_strs <- calc_avg_int_str_mult(r_model, vr_type, mat_rows, species, timestep = timestep, avgtf = avgtf, sp1_0 = sp1_0, sp2_0 = sp2_0, dstb_func = dstb_func, vrstoch_func = vrstoch_func)
    }
    m_int_strs <- calc_avg_int_str_mult(m_model, vr_type, mat_rows, species, timestep = timestep, avgtf = avgtf, sp1_0 = sp1_0, sp2_0 = sp2_0, dstb_func = dstb_func, vrstoch_func = vrstoch_func)
