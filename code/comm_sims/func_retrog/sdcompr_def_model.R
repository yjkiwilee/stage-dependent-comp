...
#' g (maturation probabilities, n-1), r (retrogression probabilities, n-1), f (fecundity, n-1),
#' and p (recruitment factor; number of new recruits per unit fecundity, n-1)
#' With n stages, the competitive coefficient matrices is a list that contains s, g, r, f, and p.
...
def_sdcomp_model <- function(n_stages, sp1_vrs, sp2_vrs, k_mat) {
  ...
  k_mat$p <- cbind(k_mat$p, matrix(rep(0, nrow(k_mat$p)), ncol = 1))
...
#' @param sp1_vrs List containing the vital rates of species 1 (must have s, g, r, f, p components)
#' @param sp2_vrs List containing the vital rates of species 2 (must have s, g, r, f, p components)
#' 
