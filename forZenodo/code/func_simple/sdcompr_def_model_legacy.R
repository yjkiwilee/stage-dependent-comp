...
vrs_to_kmat <- function(n_stages, sp1_vrs, sp2_vrs, prev_k_mat) {
  ...
  for(vr_type in names(k_mat)) {
    ...
    if(is_logistic) {
      intrcpt <- qlogis(comm_vrs) # Inverse function of logistic
    } else {
      intrcpt <- log(comm_vrs) # Inverse function of exponential
    }
...
kmat_to_vrs <- function(n_stages, k_mat) {
  ...
  for(vr_type in names(k_mat)) {
    ...
    if(is_logistic) {
      comm_vrs <- plogis(intrcpt) # Logistic function of intercept
    } else {
      comm_vrs <- exp(intrcpt) # Exponential function of intercept
    }
