...
tail(stoch_rnorms[,c(3, length(vr_names) + 3)])
...
for(dd_vr_i in 1:length(vr_names)) {
  ...
  for(sp_comb_i in 1:length(sp_combs)) {
    ...
    for(rep_i in 1:n_reps) {
      ...
      ddvr_stoch_rnorms <- stoch_rnorms[,c(dd_vr_i, length(vr_names) + dd_vr_i)] *
        vr_sd[dd_vr]
      ...
      if(file.exists(file.path("result_data",
                               paste0("sim_tseries_", dd_vr, ".csv")))) {
        write_csv(
          tseries_df,
          file.path("result_data", paste0("sim_tseries_", dd_vr, ".csv")),
          append = TRUE
        )
      } else {
        write_csv(
