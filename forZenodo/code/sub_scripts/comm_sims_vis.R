...
meanvar_df_ci <- read_csv(
  file.path("result_data","meanvar_ci.csv")
) %>%
  mutate(
...
partial_df_ci <- read_csv(
  file.path("result_data","partial_ci.csv")
) %>%
  mutate(
    foc_sp_name = factor(foc_sp_name, levels = sp_names,
                         labels = paste("C:", sp_names)),
...
mse_df_ci <- read_csv(
  file.path("result_data","mse_ci.csv")
) %>%
  mutate(
    foc_sp_name = factor(foc_sp_name, levels = sp_names,
                         labels = paste("C:", sp_names)),
...
mse_long_df_ci <- read_csv(
  file.path("result_data","mse_long_ci.csv")
) %>%
  mutate(
    foc_sp_name = factor(foc_sp_name, levels = sp_names,
                         labels = paste("C:", sp_names)),
...
mse_long_df_ci_mcomp <- read_csv(
  file.path("result_data","mse_long_ci_mcomp.csv")
) %>%
  mutate(
    foc_sp_name = factor(foc_sp_name, levels = sp_names),
...
for(var_name in var_names) {
  for(var_type in names(type_labels)) {
    ...
    ggsave(
      file.path("figures", paste0(var_name, "_", var_type, ".png")),
      plots_wrapped,
      width = 9,
      height = 12
    )
...
for(rv in partial_resp) {
  ...
  ggsave(
    file.path("figures", paste0("partial_", rv, "_mod.png")),
    partial_plot,
    width = 9,
    height = 12
  )
...
for(rv in mse_resp) {
  ...
  ggsave(
    file.path("figures", paste0(rv, ".png") ),
    mse_plot,
    width = 9,
    height = 12
  )
...
for(rv in mse_resp) {
  ...
  ggsave(
    file.path("figures", paste0(rv, "_diff.png") ),
    mse_plot,
    width = 9,
    height = 12
  )
...
for(rv in mse_resp) {
  ...
  ggsave(
    file.path("figures", paste0(rv, "_long.png") ),
    mse_plot,
    width = 9,
    height = 12
  )
...
for(rv in mse_resp) {
  ...
  ggsave(
    file.path("figures", paste0(rv, "_long_merge.png") ),
    mse_plot,
    width = 6,
    height = 5.5
  )
...
for(rv in mse_resp) {
  ...
  ggsave(
    file.path("figures", paste0(rv, "_long_envonly.png") ),
    mse_plot,
    width = 9,
    height = 12
  )
...
for(rv in mse_resp) {
  ...
  ggsave(
    file.path("figures", paste0(rv, "_long_merge_envonly.png") ),
    mse_plot,
    width = 6,
    height = 5.5
  )
...
ggsave(
  file.path("figures", "mape_example_plot.png" ),
  mse_example_plot,
  width = 7,
  height = 5.5,
  scale = 0.8
...
ggsave(
  file.path("figures", "mape_example_plot_noscale.png"),
  mse_example_plot,
  width = 7,
  height = 5.5,
  scale = 0.8
